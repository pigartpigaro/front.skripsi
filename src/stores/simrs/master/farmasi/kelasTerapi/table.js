...
export const useMasterKelasTerapiTable = defineStore('master_KelasTerapi_table', {
  state: () => ({
    items: [],
    meta: {},
    item: {},
  ...
  actions: {
    ...
    getDataTable () {
      ...
      return new Promise((resolve, reject) => {
        api
          .get('v1/simrs/farmasi/master/listkelasterapi', params)
          .then((resp) => {
            waitLoad('done')
            // console.log(resp)
            if (resp.status === 200) {
    ...
    deletesData (payload) {
      ...
      return new Promise((resolve, reject) => {
        api
          .post('v1/simrs/farmasi/master/hapuskelasterapi', params)
          .then((resp) => {
            // console.log(resp)
            notifSuccess(resp)
            this.loading = false
