...
export const useDiagnosaHDStore = defineStore('diagnosa-hd-store', {
  state: () => ({
    ...
    storePasien: useListPasienHemodialisaStore(),
    // tindakan
    // searchtindakan: '',
    // notaTindakans: [],
    // notaTindakan: 'BARU',
  ...
  actions: {
    ...
    async getData (pasien) {
      ...
      const resp = await api.get('v1/simrs/hemodialisa/layanan/diagnosa/getDiagnosaByNoreg', params)
    ...
    async simpanDiagnosa (pasien) {
      ...
      try {
        const resp = await api.post('v1/simrs/hemodialisa/layanan/diagnosa/simpandiagnosa', form)
        // console.log(resp)
        if (resp.status === 200) {
          ...
          this.storePasien.injectDataPasien(pasien?.noreg, isi, 'diagnosamedis')
          notifSuccess(resp)
    ...
    PISAH_DATA_RANAP_IGD (arr, pasien) {
      ...
      ranap?.forEach(x => {
        this.storePasien.injectDataPasien(pasien?.noreg, x, 'diagnosamedis')
      })
    ...
    async deleteDiagnosa (pasien, id) {
      const payload = { id }
      const resp = await api.post('v1/simrs/hemodialisa/layanan/diagnosa/hapusdiagnosa', payload)
      if (resp.status === 200) {
        // const storeIna = useInacbgPoli()
        this.storePasien.hapusDataInjectan(pasien, id, 'diagnosamedis')
...
if (import.meta.hot) {
  import.meta.hot.accept(acceptHMRUpdate(useDiagnosaHDStore, import.meta.hot))
}
