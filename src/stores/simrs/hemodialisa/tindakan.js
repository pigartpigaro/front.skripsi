...
export const useTindakanHemodialisaStore = defineStore('tindakan-hemodialisa-store', {
  ...
  actions: {
    ...
    async getTindakan (pasien) {
      try {
        const resp = await api.get('v1/simrs/hemodialisa/layanan/tindakan/listtindakan-hd', { params: { noreg: pasien?.noreg, kodepoli: pasien?.kodepoli } })
    ...
    async saveTindakan (pasien) {
      ...
      return new Promise((resolve, reject) => {
        api.post('v1/simrs/hemodialisa/layanan/tindakan/simpantindakan-hd', form)
          .then((resp) => {
            if (resp.status === 200) {
        ...
        api.post('v1/simrs/pelayanan/simpandokumentindakanpoli', data, {
          ...
          .then(res => {
            ...
            if (res.status === 200) {
              ...
              this.storePasien.injectDokumenTindakan(pasien, tindakan)
            }
    ...
    async hapusTindakan (pasien, id) {
      ...
      try {
        ...
        if (resp.status === 200) {
          ...
          this.storePasien.hapusDataInjectan(pasien, id, 'tindakan')
          this.setNotas(resp?.data?.nota)
          notifSuccess(resp)
        }
      }
        ...
        api.post('v1/simrs/pelayanan/simpanprocedure', this.formicd)
          .then(resp => {
            ...
            if (resp.status === 200) {
              ...
              const isi = resp?.data?.result
              this.storePasien.injectDataPasien(pasien, isi, 'prosedur')
    ...
    hapusProsedur (pasien, id) {
      ...
      return new Promise(resolve => {
        api.post('v1/simrs/pelayanan/hapusprocedure', payload)
          .then(resp => {
            this.loadingSaveIcd = false
            this.storePasien.hapusDataProsedur(pasien, id)
            const storeIna = useInacbgPoli()
            storeIna.getDataIna(pasien)
            notifSuccess(resp)
            resolve(resp)
...
if (import.meta.hot) {
  import.meta.hot.accept(acceptHMRUpdate(useTindakanHemodialisaStore, import.meta.hot))
}
