  ...
  <div>
    <div>
      <div class="row q-col-gutter-sm q-mb-sm">
        <div class="col-9">
          <app-input v-model="store.formKontrol.nosep" label="Sep digunakan" outlined readonly />
        </div>
        <div class="col-2">
          <app-btn label="Ganti SEP" @click="store.openDialogSep = true" />
        </div>
      </div>
    </div>
      ...
      <div class="row q-col-gutter-sm">
        ...
        <div class="col-5">
          <app-autocomplete :key="store.formKontrol.kodedokterdpjp" v-model="store.formKontrol.kodedokterdpjp" outlined
            label="Pilih Dokter" autocomplete="namadokter" option-label="namadokter" option-value="kodedokter"
            :loading="store.loadingJadwalDokter" :disable="store.loadingJadwalDokter" :source="store.jadwalDpjps" />
        </div>
        <div class="col-5">
        ...
        <div class="col-12">
          ...
          <div class="text-right q-gutter-sm">
            <q-btn label="Simpan" color="primary" type="submit" :loading="store.loadingSaveKontrol"
              :disable="store.loadingSaveKontrol" />
          </div>
        </div>
      </div>
    </q-form>
    <app-dialog v-model="store.openDialogSep" label="Dialog Cari SEP" @on-ok="store.openDialogSep = false">
      <template #default>
        <div class="q-pa-sm">
          <div class="row q-col-gutter-sm q-mb-sm">
            <div class="col-4">
              <app-input-date :model="store.paramListSep.tglawal" label="cari mulai tgl" outlined
                @set-model="(val) => store.paramListSep.tglawal = val" />
            </div>
            <div class="col-4">
              <app-input-date :model="store.paramListSep.tglakhir" label="cari sampai tgl" outlined
                @set-model="(val) => store.paramListSep.tglakhir = val" />
            </div>
            <div class="col-2">
              <app-btn label="cari Sep" :loading="store.loadingListSep" @click="store.getListSep(pasien)" />
            </div>
          </div>
          <div v-if="store?.listSeps?.length && !store.loadingListSep">
            <q-list separator>
              <q-item v-for="(item, i) in store.listSeps" :key="i" clickable @click="gunkanSep(item)">
                <q-item-section>
                  <q-item-label>
                    <span class="text-weight-bold">{{ item?.namaPeserta }}</span> | <span class="text-primary ">{{
                      item?.noKartu }}</span>
                  </q-item-label>
                  <q-item-label>
