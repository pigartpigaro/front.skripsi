<template>
  ...
  <div class="q-pa-sm">
    <div
      v-if="!terapeutiks?.length"
      class="text-center"
    >
      <em>Data Terapeutik Belum Ada</em>
    </div>
    <div v-else>
      <div
        v-for="(row, i) in terapeutiks"
        :key="row.id"
        class="row"
...
const terapeutiks = computed(() => {
  const arr = store?.diagnosa?.intervensis?.filter(x => x?.group === 'terapeutik' && x?.mdiagnosakeperawatan_kode === store?.diagnosa?.kode)
  return arr
})
...
function setScope(scope) {
  scope.set()
  // console.log(scope)
  store.saveIntervensi('terapeutik', null)
}
function editIntervensi(scope, row) {
  scope.set()
  store.saveIntervensi('terapeutik', row)
}
