<template>
  <q-card>
    <q-card-section>
      ...
      <div v-if="props.data?.data?.racikan?.length === 0">
        <app-no-data />
      </div>
      <div v-for="(item,i) in props.data?.data?.racikan" :key="item">
            ...
            @click="()=>{
              if (parseFloat(item?.harga) !== parseFloat(cekHarga(item)?.harga)) {
                $q.dialog({
                  ...
                  emits('ubahharga',{item,penerimaan:cekHarga(item),harga:cekHarga(item)?.harga,tipe:'racikan',id:item?.id})
                })
              }
            }"
          >
