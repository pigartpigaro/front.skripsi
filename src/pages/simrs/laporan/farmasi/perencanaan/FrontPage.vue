  ...
  <div class="bg-white q-pa-xs">
          ...
          <div class="row q-col-gutter-sm q-my-sm">
            ...
            <div class="col-1">
              <app-autocomplete v-model="store.params.jenis" label="Jenis" autocomplete="nama" option-label="nama"
                option-value="value" outlined hide-dropdown-icon :source="store.optionJenis" :loading="store.loading"
                :disable="store.loading || !!store.ketProses" @update:model-value="setJenis" />
            </div>
            <div class="col-2">
              <app-autocomplete v-model="store.params.reference" label="Referensi Tanggal" autocomplete="value"
                option-label="label" option-value="value" outlined hide-dropdown-icon :source="store.optionReferences"
                :loading="store.loading" :disable="store?.loading || !!store?.ketProses" @update:model-value="() => {
                  store.items = []
                  store.meta = {}
            ...
            <div class="col-2">
              <app-input-date-human :model="store.tanggal.from" label="dari tanggal" outlined
                :disable="store?.loading || !!store?.ketProses" @db-model="(val) => {
                  // console.log('set to', val);
                  store.setParams('from', val)
                }" @set-display="(val) => {
                  // console.log('set disp', val);
                  store.tanggal.from = val

                }" />
            </div>
            <div class="col-2">
              <app-input-date-human :model="store.tanggal.to" label="sampai tanggal" outlined
                :disable="store?.loading || !!store?.ketProses" @db-model="(val) => {
                  // console.log('set to', val);
    ...
    <div id="printMe">
      ...
      <div class="row justify-center f-16 text-weight-bold q-my-sm">
        Laporan Perencanaan Farmasi Periode Tanggal {{ store.params.reference === 'rencana' ? 'Perencanaan' :
          'Penerimaan' }}
      </div>
      <div class="row justify-center f-14 text-weight-bold q-my-sm">
        {{ store.tanggal.from }} s.d. {{ store.tanggal.to }}
      </div>
      <div>
