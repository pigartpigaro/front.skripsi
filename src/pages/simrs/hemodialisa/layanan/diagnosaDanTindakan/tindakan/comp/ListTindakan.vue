      ...
      <q-card flat bordered class="q-mb-xs" v-for="(item, i) in filterredTableByTgl" :key="i">
        <q-item class="list-move">
          <q-item-section>
            ...
            <q-item-label lines="3" class="f-10 text-italic">
            ...
            <q-item-label lines="2" class="f-12 text-italic text-accent">
              ...
              (<span v-for="(pel, x) in setPelaksana(item)?.pelaksanaSatu" :key="x" class="">{{ namaPetugas(pel) }},
              </span>)
              <span class=""> / </span> <b>Pel2 </b>
              (<span v-for="(pel, y) in setPelaksana(item)?.pelaksanaDua" :key="y" class="">{{ namaPetugas(pel) }},
              </span>)
            </q-item-label>
          </q-item-section>
          ...
          <q-item-section side top>
            ...
            <q-item-label v-if="item.gambardokumens?.length">
              <q-btn label="lihat dokumen" class="q-px-md" dense color="info" size="sm" @click="lihatDokumen(item)" />
            </q-item-label>
          </q-item-section>
        </q-item>
